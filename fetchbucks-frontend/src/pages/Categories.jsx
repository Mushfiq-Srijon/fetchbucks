    ...
    <div className="fb-page-cols" style={{ display: 'flex', gap: 28, alignItems: 'flex-start' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {showForm && (
          <div className="fb-slide-down" style={{ background: '#0d1117', border: '1px solid #1a2535', borderRadius: 16, padding: '26px 28px', marginBottom: 22, position: 'relative', overflow: 'hidden' }}>
            {/* accent line — dynamically uses chosen color */}
            ...
            <div style={{ marginBottom: 22 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#3a5270', marginBottom: 8 }}>
                Name
              </label>
            ...
            {/* Color */}
            <div style={{ marginBottom: 22 }}>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#3a5270', marginBottom: 12 }}>
                Color
              </label>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 16 }}>
                {PRESETS.map(c => (
                  <button key={c} className="fb-swatch" onClick={() => setForm({ ...form, color: c })}
                    style={{
                      background: c,
                      boxShadow: form.color === c ? `0 0 0 2px #0d1117, 0 0 0 4px ${c}` : 'none',
                      transform: form.color === c ? 'scale(1.18)' : 'scale(1)',
                    }} />
                ))}
                {/* Rainbow "custom" swatch */}
                <div style={{ position: 'relative', width: 30, height: 30 }}>
                  <button className="fb-swatch"
                    onClick={() => colorRef.current?.click()}
                    style={{
                      background: 'conic-gradient(#f43f5e,#f97316,#eab308,#22c55e,#4f9cf9,#8b5cf6,#f43f5e)',
                      boxShadow: !PRESETS.includes(form.color) ? `0 0 0 2px #0d1117, 0 0 0 4px ${form.color}` : 'none',
              ...
              <div style={{ padding: '14px 18px', borderRadius: 12, background: '#080c12', border: '1px solid #111c27', display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, flexShrink: 0, background: `${form.color}20`, border: `1px solid ${form.color}35`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, transition: 'all 0.25s' }}>
                  ◈
                </div>
                <div style={{ flex: 1 }}>
                ...
                <span style={{ fontSize: 12, fontWeight: 600, padding: '4px 12px', borderRadius: 20, background: `${form.color}18`, color: form.color, border: `1px solid ${form.color}28`, transition: 'all 0.25s' }}>
                  {form.name || 'tag'}
                </span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button className="fb-btn" onClick={handleSubmit}
                style={{ background: `linear-gradient(135deg,${form.color},${form.color}bb)`, color: '#fff', boxShadow: `0 4px 14px ${form.color}40`, transition: 'all 0.2s' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(270px,1fr))', gap: 14 }}>
            {categories.map((cat, i) => (
              <div key={cat.id} className="fb-cat-card fb-fade-up" style={{ animationDelay: `${i * 0.06}s` }}>
                {/* Swatch */}
                <div style={{ width: 48, height: 48, borderRadius: 12, flexShrink: 0, background: `${cat.color}20`, border: `1px solid ${cat.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, boxShadow: `0 0 16px ${cat.color}12` }}>
                  ◈
                </div>
                {/* Info */}
                <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div className="fb-page-sidebar" style={{ width: 268, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {[
          ...
          { label: 'Most Recent', value: categories.length ? categories[categories.length - 1].name : '—', unit: 'last added', accent: '#8b5cf6', small: true },
        ...
        {categories.length > 0 && (
          <div style={{ padding: '20px 22px', borderRadius: 14, background: '#0d1117', border: '1px solid #111c27' }}>
            <div style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#3a5270', marginBottom: 16 }}>
              Your Palette
            </div>
